...
class RenderFlatWire(block: Block) extends StaticBlockRenderer(block) {
	...
	def render(wire: BlockWire, pos: Vector3) {
	...
	def renderBreakingOverlay(icon: IIcon, wire: BlockWire) {

      ...
      if (hit != null)
      {
        if (hit.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
        {
          ...
          if (hitTile.isInstanceOf[ILaserHandler])
          {
            ...
            {
              ResonantInduction.proxy.renderLaser(world, renderStart, hitVec, color, energy)
            }
          }
          ...
          else
          {
             ...
             */
            ResonantInduction.proxy.renderScorch(world, hitVec - (direction * 0.02), hit.sideHit)
            ResonantInduction.proxy.renderBlockParticle(world, hitVec, hitBlock, hit.sideHit)
          }
        }
        else if (hit.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY)
        {
          ...
          ResonantInduction.proxy.renderLaser(world, renderStart, new Vector3(hit.hitVec), color, energy)
        }

        return
      }

      ResonantInduction.proxy.renderLaser(world, renderStart, maxPos, color, energy)
    }
  }
